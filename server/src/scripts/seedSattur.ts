...
// Function to slightly offset coordinates around Sattur (within 1-3 km)
function offsetCoord(base: number, maxKm = 0.02): number {
...
async function seedSatturItems() {
  try {
    ...
    for (let idx = 0; idx < satturMerchants.length; idx++) {
      ...
      // Find or create MerchantProfile
      ...
      if (!merchant) {
        merchant = await MerchantProfile.create({
          ...
          location: {
            type: 'Point',
            coordinates: [mData.lng, mData.lat], // [longitude, latitude]
          },
          operatingHours: [
        ...
        // Ensure coordinates are updated to Sattur
        merchant.location = {
          type: 'Point',
      ...
      // Insert items for this merchant
      for (const item of mData.items) {
        // Delete existing items with same title & merchant
        ...
        const listing = await Listing.create({
          ...
          claimWindowStart: new Date(now.getTime() - 1000 * 60 * 30), // 30 mins ago
          claimWindowEnd: new Date(now.getTime() + 1000 * 60 * 60 * 12), // 12 hours from now
          status: 'active',
        });
    ...
    console.log(`\n🎉 Successfully added ${createdListings.length} Sattur items across ${satturMerchants.length} merchants!`);
    console.log(`📍 Location centered around Sattur, Tamil Nadu (Pincode: 626203, Lat: ${SATTUR_LAT}, Lng: ${SATTUR_LNG})`);
