...
import { Listing, LISTING_CATEGORIES } from '../models/Listing';

// Load .env from server root
dotenv.config({ path: path.join(__dirname, '../../.env') });

// Category-specific image pools — each category has unique photos
...
function getRandomTags(): string[] {
  // 50% chance to have no tags
  if (Math.random() > 0.5) return [];
  // Otherwise pick 1-2 random tags
...
async function getLocation(): Promise<{ lat: number; lng: number }> {
  // Check for command line args --lat and --lng
  ...
  console.log('🌍 Fetching your current location automatically via IP...');
  try {
    const response = await fetch('https://ipapi.co/json/');
    const data = (await response.json()) as IpApiResponse;
    if (data.latitude && data.longitude) {
      console.log(`📍 Found location: ${data.city}, ${data.country_name} (${data.latitude}, ${data.longitude})`);
...
// Randomize coordinates slightly (approx within 5km radius)
function randomizeCoordinate(coord: number, maxOffset = 0.045): number {
...
async function runSeed() {
  try {
    ...
    // Also delete the base local user just in case
    ...
    const user = await User.create({
      ...
      passwordHash: 'dummy_hash', // Not meant to be logged into
      role: 'merchant',
      emailVerified: true
    });
    ...
    for (const category of LISTING_CATEGORIES) {
      for (let i = 1; i <= 5; i++) {
        ...
        // We need 1 merchant per distinct location if we want scattered pins on the map
        // Actually, the Map groups by merchantId. If we want scattered pins, we should create multiple merchants!
        // Let's create a new merchant for each item to scatter them around.
        ...
        const m = await MerchantProfile.create({
          ...
          location: {
            type: 'Point',
            coordinates: [itemLng, itemLat] // [longitude, latitude]
          },
        ...
        listings.push({
          ...
          claimWindowStart: new Date(now.getTime() - 1000 * 60 * 60 * 2), // 2 hours ago
          claimWindowEnd: new Date(now.getTime() + 1000 * 60 * 60 * (4 + Math.random() * 8)), // 4-12 hours from now
          status: 'active'
        });
      }
    }
